...
COLORS = (BLACK, GRAY, BLUE, GREEN, RED, YELLOW, CYAN, MAGENTA)
LIGHTCOLORS = (BLACK, WHITE, LIGHTBLUE, LIGHTGREEN, LIGHTRED, LIGHTYELLOW,
...
explore_change = 1.0
#순서대로
#height_sum, diff_sum, max_height, min_diff, holes, wall_meet
input_key=[]
#'left','right','space','up'
...
def run_game(weights, explore_change):
    ...
    while True:  # game loop
        ...
        if (event =='left') and is_valid_position(
        ...
        elif (event == 'right') and is_valid_position(
                  board, falling_piece, adj_x=1):
            falling_piece['x'] += 1
            moving_right = True
            moving_left = False
...
def get_new_piece():
    ...
    color_index = PIECE_index.index(shape)
...
def get_parameters(board):
    ...
    # 총 높이의 합, 높이 편차의 절댓값의 합, 최고높이, 아래 뚫려있는 구멍의 합 을 리턴함
    ...
    # Calculate the minimum diff
    min_diff = max(heights)- min(heights)

    # Calculate wall meet
    wall_meet = 0
    for i in range(0, BOARDWIDTH):
        if int(board[i][BOARDHEIGHT-1]) > 0:
            wall_meet+=1

    for j in range(0, BOARDHEIGHT-1):
        if int(board[0][j]) > 0:
            wall_meet+=1
        if int(board[BOARDWIDTH-1][j]) > 0:
            wall_meet+=1
    ...
    for i in diffs:
        diff_sum += abs(i) * abs(i)
    diff_sum = diff_sum/len(diffs)
    diff_sum**0.5

    return height_sum, diff_sum, max_height, min_diff, holes, wall_meet
...
def get_expected_score(test_board, weights):
    ...
    height_sum, diff_sum, max_height, min_diff, holes, wall_meet = get_parameters(test_board)
    A = weights[0]
    B = weights[1]
    C = weights[2]
    D = weights[3]
    E = weights[4]
    F = weights[5]
    test_score = float( A * height_sum + (B * diff_sum) + (C * max_height) + (D * min_diff) + (E * holes) + (F * wall_meet))
    return test_score
...
def simulate_board(test_board, test_piece, move):
    ...
    height_sum = get_parameters(test_board)[0]
    one_step_reward = test_lines_removed * test_lines_removed - (height_sum-reference_height)
    return test_board, one_step_reward


def find_best_move(board, piece, weights, explore_change):
    # rot과 sideways는 블럭이 이동할수 있는 총 경우의 수를 분석
    # rot : 방향 경우의수
    # sideways : 좌우 이동 경우의 수
    move_list = []
    score_list = []
...
def make_move(move):
    ...
    if rot != 0:
        ...
        if sideways == 0:
            input_key.append('space')
            #pyautogui.press('space')
        if sideways < 0:
            input_key.append('left')
            #pyautogui.press('left')
            sideways += 1
        if sideways > 0:
            input_key.append('right')
